<form action="index.php?action=edit-operation&amp;id=<?= $data['id'] ?>" method="post"> 
        ...
        <tr class="<?= $color; ?> tr<?= $data['id'] ?>" id="<?= $data['id'] ?>">
            ...
            <td id="td-actions">
                    ...
                    <input type="submit" class="round-big-btn green sub-edit-operation appear-on-edit" id="sub-confirm-edit-<?= $data['id'] ?>" name="sub-edit-operation" value="confirm" hidden ></input>
                </form>
                    ...
                    <input type="submit" class="round-little-btn neutral sub-edit disappear-on-edit" id="sub-edit-<?= $data['id'] ?>" name="sub-able-edit" value="edit" onclick="switchEditModeForLine(<?= $data['id'] ?>)"></input>

                    <!-- SUB CANCEL EDIT hidden-->
                    <!-- visible only if line is being edited -->
                    <input type="submit" class="round-big-btn red sub-cancel-edit appear-on-edit" id="sub-cancel-edit-<?= $data['id'] ?>" name="sub-cancel-edit" onclick="switchEditModeForLine(<?= $data['id'] ?>, false)" value="cancel" hidden>

                    <!-- SUB SUPPR OP -->
                    <!-- made appear a popup who ask confirm for delete the line -->
                    <input type="submit" class="round-little-btn red sub-suppr disappear-on-edit" id="suppr-<?= $data['id'] ?>" name="sub-suppr-op" value="X" onclick="return displayHiddePopupConfirm('You will delete the entry n° ' + <?= $data['id'] ?> + ' ?', '#sub-suppr-' + <?= $data['id']; ?>)" >
                    ...
                    <form action="index.php?action=suppr-operation&amp;id=<?= $data['id'] ?>" method="post">
                        ...
                        <input type="submit" class="round-little-btn red sub-suppr disappear-on-edit" id="sub-suppr-<?= $data['id'] ?>" name="sub-suppr-op" value="X" hidden>
                </form>
            </td>
